...
export function useQuery<T>(key: string, fetcher: () => Promise<T>, options: QueryOptions = {}): QueryResult<T> {
  ...
  useEffect(() => { setData(readCache<T>(key)); const unsubscribe = subscribeCache(key, () => { setData(readCache<T>(key)); if (enabled && cacheAge(key) > staleTime) void execute(); }); if (enabled && (readCache<T>(key) === undefined || cacheAge(key) > staleTime)) void execute(); return unsubscribe; }, [enabled, execute, key, staleTime]);
